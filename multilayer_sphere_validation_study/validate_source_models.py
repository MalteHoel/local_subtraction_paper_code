...
output_folder = configs.get('io', 'output_folder')
...
#########################################
# create driver and compute transfer matrices
#########################################
...
if numerical_simulation_needed:
  ...
  if do_eeg:
    eeg_dependencies_change_time = max(os.path.getctime(os.path.join(input_folder, 'sensors', 'electrodes.txt')), \
                                       os.path.getctime(os.path.join(input_folder, 'volume_conductor', 'conductivities.txt')), \
                                       os.path.getctime(os.path.join(input_folder, 'volume_conductor', 'mesh.msh')))
    
    compute_eeg_transfer = (not os.path.exists(os.path.join(output_folder, 'transfermatrices', 'eeg_transfer_matrix.npy'))) or \
                           (os.path.getctime(os.path.join(output_folder, 'transfermatrices', 'eeg_transfer_matrix.npy')) < eeg_dependencies_change_time)

  if do_meg:
    meg_dependencies_change_time = max(os.path.getctime(os.path.join(input_folder, 'sensors', 'coils.txt')), \
                                       os.path.getctime(os.path.join(input_folder, 'sensors', 'projections.txt')), \
                                       os.path.getctime(os.path.join(input_folder, 'volume_conductor', 'conductivities.txt')), \
                                       os.path.getctime(os.path.join(input_folder, 'volume_conductor', 'mesh.msh')))
    
    compute_meg_transfer = (not os.path.exists(os.path.join(output_folder, 'transfermatrices', 'meg_transfer_matrix.npy'))) or \
                           (os.path.getctime(os.path.join(output_folder, 'transfermatrices', 'meg_transfer_matrix.npy')) < meg_dependencies_change_time)
...
if compute_eeg_transfer:
  ...
  np.save(os.path.join(output_folder, 'transfermatrices', 'eeg_transfer_matrix.npy'), transfer_matrix)
  with open(os.path.join(output_folder, 'transfermatrices', 'eeg_transfer_matrix_computation_information.json'), 'w') as outfile:
...
if do_meg and (numerical_simulation_needed or not skip_analytical_meg):
  ...
  coils = dp.read_field_vectors_3d(os.path.join(input_folder, 'sensors', 'coils.txt'))
  projections = dp.read_projections_3d(os.path.join(input_folder, 'sensors', 'projections.txt'))
  if numerical_simulation_needed:
...
if compute_meg_transfer:
  ...
  np.save(os.path.join(output_folder, 'transfermatrices', 'meg_transfer_matrix.npy'), transfer_matrix)
  with open(os.path.join(output_folder, 'transfermatrices', 'meg_transfer_matrix_computation_information.json'), 'w') as outfile:
    outfile.write(json.dumps(computation_information))
  print(f"Transfer matrix saved")
elif do_meg:
...
if do_eeg and numerical_simulation_needed:
  ...
  transfer_matrix = np.load(os.path.join(output_folder, 'transfermatrices', 'eeg_transfer_matrix.npy'))
  # radial dipoles
  print("Simulating radial dipoles")
  radial_dipole_filenames = os.listdir(os.path.join(input_folder, 'dipoles', 'radial'))
  for filename in radial_dipole_filenames:
    dipoles = dp.read_dipoles_3d(os.path.join(input_folder, 'dipoles', 'radial', filename))
    for source_model in source_models_for_numerical_simulation:
      ...
      np.savetxt(os.path.join(output_folder, 'results', 'eeg', 'radial', 'numerical_solution', f'{source_model}_numerical_solution_{filename}'), np.array(numerical_solution), fmt='%.18f', delimiter = ' ')
  # tangential dipoles
  print("Simulating tangential dipoles")
  tangential_dipole_filenames = os.listdir(os.path.join(input_folder, 'dipoles', 'tangential'))
  for filename in tangential_dipole_filenames:
    dipoles = dp.read_dipoles_3d(os.path.join(input_folder, 'dipoles', 'tangential', filename))
    for source_model in source_models_for_numerical_simulation:
      driver_config['source_model'] = source_model_config_database[source_model]
      start_time = time.time()
      numerical_solution, computation_information = meeg_driver.applyEEGTransfer(transfer_matrix, dipoles, driver_config)
      print(f"Computing forward solutions for {source_model} took {time.time() - start_time} seconds")
      np.savetxt(os.path.join(output_folder, 'results', 'eeg', 'tangential', 'numerical_solution', f'{source_model}_numerical_solution_{filename}'), np.array(numerical_solution), fmt='%.18f', delimiter = ' ')
...
if do_meg and numerical_simulation_needed:
  ...
  transfer_matrix = np.load(os.path.join(output_folder, 'transfermatrices', 'meg_transfer_matrix.npy'))
  # tangential dipoles
  print("Simulating tangential dipoles")
  tangential_dipole_filenames = os.listdir(os.path.join(input_folder, 'dipoles', 'tangential'))
  for filename in tangential_dipole_filenames:
    ...
    for source_model in source_models_for_numerical_simulation:
      ...
      print(f"Computing forward solutions for {source_model} took {time.time() - start_time} seconds")
      np.savetxt(os.path.join(output_folder, 'results', 'meg', 'numerical_solution', f'{source_model}_numerical_solution_{filename}'), np.array(numerical_solution), fmt='%.18f', delimiter = ' ')
...
if do_eeg and not skip_analytical_eeg:
  ...
  for filename in radial_dipole_filenames:
    dipoles = dp.read_dipoles_3d(os.path.join(input_folder, 'dipoles', 'radial', filename))
    with open(os.path.join(output_folder, 'results', 'eeg', 'radial', 'analytical_solution', f'analytical_solution_{filename}'), mode='w') as outputfile:
  ...
  for filename in tangential_dipole_filenames:
    dipoles = dp.read_dipoles_3d(os.path.join(input_folder, 'dipoles', 'tangential', filename))
    with open(os.path.join(output_folder, 'results', 'eeg', 'tangential', 'analytical_solution', f'analytical_solution_{filename}'), mode='w') as outputfile:
      for count, dipole in enumerate(dipoles):
        analytical_solution = sp.analytic_solution(radii, sphere_center, conductivities, electrodes_simbio, np.array(dipole.position()).tolist(), np.array(dipole.moment()).tolist())
        mean = sum(analytical_solution) / float(len(analytical_solution))
        analytical_solution = [x - mean for x in analytical_solution]
...
if do_meg and not skip_analytical_meg:
  ...
  for filename in tangential_dipole_filenames:
    dipoles = dp.read_dipoles_3d(os.path.join(input_folder, 'dipoles', 'tangential', filename))
    with open(os.path.join(output_folder, 'results', 'meg', 'analytical_solution', f'analytical_solution_{filename}'), mode='w') as outputfile:
      for count, dipole in enumerate(dipoles):
...
def create_comparisons(source_model, basefolder, orientation_tag):
  basename_files = os.listdir(os.path.join(input_folder, 'dipoles', orientation_tag))
...
if comparisons_needed:
  ...
  for source_model in source_models_for_comparison:
    ...
    if do_eeg:
      ...
      create_comparisons(source_model, os.path.join(output_folder, 'results', 'eeg', 'radial'), 'radial')
      
      # tangential
      print("Comparing tangential solutions")
      create_comparisons(source_model, os.path.join(output_folder, 'results', 'eeg', 'tangential'), 'tangential')
    ...
    if do_meg:
      ...
      #tangential
      print("Comparing tangential solutions")
      create_comparisons(source_model, os.path.join(output_folder, 'results', 'meg'), 'tangential')
...
def create_boxplots(dataframe, basefolder, modality_tag, orientation_tag):
  ...
  plt.savefig(os.path.join(basefolder, 'boxplots', f'relative_error_{modality_tag}_{orientation_tag}_{"_".join(source_models_for_boxplot)}_{"_".join(eccentricity_selection)}.png'))
  plt.clf()
  ...
  plt.savefig(os.path.join(basefolder, 'boxplots', f'rdm_{modality_tag}_{orientation_tag}_{"_".join(source_models_for_boxplot)}_{"_".join(eccentricity_selection)}.png'))
  plt.clf()
  ...
  plt.savefig(os.path.join(basefolder, 'boxplots', f'lnMAG_{modality_tag}_{orientation_tag}_{"_".join(source_models_for_boxplot)}_{"_".join(eccentricity_selection)}.png'))
  plt.clf()
...
if boxplots_needed:
  ...
  if do_eeg:
    for eccentricity_selection in eccentricity_sets:
      # radial
      dataframe_eeg_radial = create_dataframe(eccentricity_selection, source_models_for_boxplot, os.path.join(output_folder, 'results', 'eeg', 'radial'), 'radial')
      create_boxplots(dataframe_eeg_radial, os.path.join(output_folder, 'results', 'eeg', 'radial'), 'eeg', 'radial')
      
      # tangential
      dataframe_eeg_tangential = create_dataframe(eccentricity_selection, source_models_for_boxplot, os.path.join(output_folder, 'results', 'eeg', 'tangential'), 'tangential')
      create_boxplots(dataframe_eeg_tangential, os.path.join(output_folder, 'results', 'eeg', 'tangential'), 'eeg', 'tangential')

  # meg
  if do_meg:
    for eccentricity_selection in eccentricity_sets:
      #tangential
      dataframe_meg_tangential = create_dataframe(eccentricity_selection, source_models_for_boxplot, os.path.join(output_folder, 'results', 'meg'), 'tangential')
      create_boxplots(dataframe_meg_tangential, os.path.join(output_folder, 'results', 'meg'), 'meg', 'tangential')
