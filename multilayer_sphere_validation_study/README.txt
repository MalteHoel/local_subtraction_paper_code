...
2) Prepare the input data
  ...
  Here, ... stands for files named "ecc_0.x_[radial|tangential].txt", where x is some number. The "input_data_example" directory illustrates the described format. Note that the purpose of the example data in "input_data_example" is to quickly test if the DUNEuro installation and the validation script are working. For a proper simulation, please use one of the higher-resolution meshes from the paper. 
